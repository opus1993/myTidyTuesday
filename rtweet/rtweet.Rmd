...
Unfortunately, the process currently works properly only in linux environments, and not in Windows. Closely inspect the token created as follows:
...
@Case_IH is the global twitter handle of one of the agriculture brand handles for CNHIndustrial. Their most recent 3200 tweets can be pulled from the API as

```{r}
case_ih_timeline <- get_timeline("case_ih", n=3200, token = token) %>%
    mutate(week = as.Date(floor_date(created_at, "week", week_start = 1)))
```
Plots describing some measures of @Case_IH activity:
...
Another approach to showing the most popular tweet of this year in a browser:
