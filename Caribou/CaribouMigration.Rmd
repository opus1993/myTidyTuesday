...
> Credit:  Cedric Scherer crafts magnificent visuals, including this one. See https://github.com/Z3tt/TidyTuesday
...
sysfonts::font_add_google("Lilita One", "Lilita One")
...
rds <- here::here("Caribou", "data", "canada_treecover_agg.Rds")

if (!file.exists(rds)) {
  raster_canada_120 <-
    raster(here::here("Caribou", "data", "Hansen_GFC2015_treecover2000_60N_120W.tif")) %>%
    aggregate(., fact = 25)

  raster_canada_130 <-
    raster(here::here("Caribou", "data", "Hansen_GFC2015_treecover2000_60N_130W.tif")) %>%
    aggregate(., fact = 25)
...
ggsave(here::here("Caribou", "2020_26_Caribous_a_map.pdf"),
...
ggsave(here::here("Caribou", "2020_26_Caribous_b_herds2.pdf"),
...
d <-
  ...
  ggplot() +
    ...
    #   title = "7 Years of Woodland Caribou Movement in Northern British Columbia"
      caption = "**C)** Northern Caribous concentrate their core range in forest habitats to obtain forage and shelter. They feed<br>primarily on lichens in winter that are found in high-elevation windswept ridges, subalpine and low-elevation<br>forests used by caribou (Jones *et al.* 2004). In summer, a wide variety of shrubs, forbs, and grasses serves as food<br>source. Shown are hered core areas, based on all cow locations. Green cells represent tree cover of 75% or more."
    )

ggsave(here::here("Caribou", "2020_26_Caribous_c_density.pdf"),
       width = 9.7, height = 12, device = cairo_pdf)
```
...
ggsave(here::here("Caribou", "2020_26_Caribous_full.pdf"),
       width = 34, height = 13, device = cairo_pdf)

## convert to png
pdfs <- list.files(here::here("Caribou"), pattern = "*.pdf")
#setwd(here::here("plots", "2020_26"))
for(pdf in pdfs) {
  pdftools::pdf_convert(pdf = here::here("Caribou", pdf),
              format = "png", dpi = 180)
}
```
