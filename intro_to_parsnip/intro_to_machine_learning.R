...
# Pull in functions
source("./intro_to_parsnip/scripts/plot_price_vs_weight.R")
source("./intro_to_parsnip/scripts/calc_metrics.R")
source("./intro_to_parsnip/scripts/plot_predictions.R")

# 2.0 Data Setup ----

# Price, Model, Product Category, and Weight
# - Web Scraped from https://www.cannondale.com/en/USA
price_vs_weight_tbl <- read_csv("./intro_to_parsnip/data/price_vs_weight_tbl.csv") 
...
engineered_features_tbl <- read_csv("./intro_to_parsnip/data/engineered_features.csv")
...
pricing_model_tbl <- price_vs_weight_tbl %>%
    ...
    filter(!(ProductFamily %in% c("Trail", "TT and TRI"))) %>% mutate(ModelBase = as_factor(ModelBase))
