...
Let's take a similar approach and sum up the facility occupancy
...
What is the distribution of rates across the network each month?
...
shelters %>% 
  ...
  annotate("text", 
           x = 7.8, 
           y = 1.63, 
           label = "Family Residence - Main Building") +
  labs(
...
Some facilities are nearly always over capacity every month.
...
What patterns do we see by month?

```{r}
shelters %>%
  mutate(month = month(occupancy_date, abbr = TRUE), 
  ...
  ggplot(aes(month, rate, color = factor(year,
                                         levels = c("2019","2018")))) +
  geom_line(size = 2) +
  scale_y_continuous(labels = scales::percent_format(accuracy = .1))+
  labs(
    title = "Monthly Average Occupancy Rate across Toronto's Homeless Shelter System", y = NULL, x = NULL,
...
What other patterns do we see each day? Start with the minimum temperature. Note that the weather file earliest data is March of 2018.
...
Setting the breakpoint for over capacity at 0.999 yields a set that is largely balanced between response classes.
