...
(downsample_rec <- token_rec %>% 
  step_downsample(genre1))
...
(filter_downsample_rec <- downsample_rec %>%
  step_stopwords(description) %>%
...
(embeddings_downsample_rec <- downsample_rec %>% 
...
(embeddings_smote_rec <- smote_rec %>% 
   textrecipes::step_word_embeddings(description,
                                     embeddings = textdata::embedding_glove27b()) %>% 
   step_tokenize(cast,
                 token = "regex",
...
(wfs <- 
   bind_rows(workflow_set(
              preproc = list(filter_smote = filter_smote_rec,
                             filter_upsample = filter_upsample_rec,
                             filter_downsample = filter_downsample_rec,
                             embeddings_smote = embeddings_smote_rec,
                             embeddings_downsample = embeddings_downsample_rec,
                             embeddings_upsample = embeddings_upsample_rec
              ),
...
And we see we have our twelve different workflows set up. 
...
The embeddings pre-processor gives a big boost in predictive performance. The bayes models train quickly, but fall apart because the model assumes independence of the features and does not handle interactions.
