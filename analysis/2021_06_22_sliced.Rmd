...
We have basic shell commands available to interact with Kaggle.
...
What imputations may be required?
...
The time series by year and within year
...
train_df %>%
  ...
  labs(x = "Year", y = NULL,
       title = "% days rain (tomorrow) in year")
```

## Numeric Features

filter out postal codes, product Id's, look for other impostor numeric variables

consider log or inverse transformations
...
train_df %>%
  ...
  ggplot() +
  geom_histogram(mapping = aes(x = value, fill = rain_tomorrow)) +
...
train_df %>%
  ...
  labs(x = "",
       ...
       color = "Time",
       caption = "DataViz code by David Robinson")
```
...
* If pressure decreased, slightly likely to rain
* If temp increased, less likely to rain
* Little effect of change in wind speed or cloud cover from 9am to 3pm
...
Let's look each modeling technique individually, glmnet first.
...
cv_res_xgboost <-
  ...
  tune_grid(
    resamples = train_folds,
    control = control_stack_resamples(),
    grid = crossing(mtry = c(15,22),
                    trees = seq(500, 6000, 500)),
    metrics = mset
  )
...
A quick confirmation look at the predictions on the held out data
...
Make the submission to the Kaggle board

```{r, eval = FALSE}
shell(glue::glue('kaggle competitions submit -c { competition_name } -f { path_export } -m "My Submission Message"'))
```

I am not entirely satisfied with the results. But on the other hand, we only had two hours.
