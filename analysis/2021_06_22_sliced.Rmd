...
```{r}
train_df %>%
  group_by(location = fct_lump(location, 40)) %>%
  summarize_rainfall() %>%
...
train_df %>%
  ...
  labs(title = "Numeric Feature Histogram Distributions",
       subtitle = "<span style = 'color:#00BFC4'>Rainy days</span> and <span style = 'color:#F8766D'> not rainy days</span>",
       x = NULL, y = NULL)
...
spread_time %>%
  ...
  geom_point() +
  geom_smooth(aes(color = rain_tomorrow), method = "lm", show.legend = FALSE) +
...
A quick look at the pre-processed training data set:
...
When mixture = 1 the glmnet model is pure lasso and without any ridge terms.
