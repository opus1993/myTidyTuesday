...
```{r setup, class.output="scroll-100"}
...
train_df %>% 
  ...
  labs(title = "Transportation leads")
...
train_df %>% 
  ...
  labs(title = "NAICS codes starting with 48")
...
train_df %>% 
  ...
  labs(title = "NAICS codes starting with 7222")
...
We will split `train_df` randomly into a training portion (90%) and a labeled testing portion (10%). We will divide the training portion into 5-fold cross validation sets and stratify on the `default_flag` to build models that are less likely to over-fit.  
...
We are going to start by taking a survey of a handful of models that train fast, have little tuning required, and are generally interpretable. The pre-processor, or {`tidymodels`} `recipe()`, transforms the dataframe into formats that machine learning algorithms can work with. 

Our strategy here is to pre-process at three levels. The first, `classification_rec` is a raw dataset with all categorical predictors intact. The second, `dummies_classification_rec` tunes a threshold for lumping parameters together and then creates dummy variables from the categoricals so everything is numeric. With so many category levels, this training set could be thousands of columns wide, and sparse. There is a cardinality tradeoff here, where each additional column may not contribute much to the model. The third pre-processor works as a sort of supervised learning version of principal components analysis, called [Partial Least Squares](https://www.tmwr.org/dimensionality.html#partial-least-squares), that simultaneously maximize the variation in the predictors while also maximizing the relationship between those components and the outcome.
...
We are going to quickly race through a grid of of parameter combinations. I will set reasonable parameters ranges for the categorical variable lumping and the number of PLS components.
...
regression_fit %>% 
  ...
  ggplot(aes(V1, Feature)) +
    geom_col() +
  labs(title = "Regression Feature Importance",
       y = NULL, x = NULL)
...
```{r, eval = FALSE}
augment(regression_fit, augment(classification_fit, competition_submission_df) %>% 
                        mutate(.pred = if_else(default_flag == "default", .pred, 0))) 
