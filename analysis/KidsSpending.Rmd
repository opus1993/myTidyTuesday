...
```{r prep, message=FALSE, warning=TRUE}
## packages
library(workflowr)
library(tidyverse)
library(geojsonio)
...
ggsave(filename = "docs/assets/kidsspending.pdf",plot = p1,  width = 12, height = 7.402, device = cairo_pdf)

pdf_convert(pdf = "docs/assets/kidsspending.pdf", 
            filenames = "docs/assets/kidsspending.png",
            format = "png", dpi = 500)
```

```{r}
knitr::include_graphics("assets/kidsspending.png", error = FALSE)
```