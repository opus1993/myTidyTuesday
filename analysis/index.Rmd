...
library(kableExtra)
library(stringr)
library(funspotr)  # install with install_github("brshallo/funspotr")
})
...
A list of all functions used across all documents:

```{r}
#| label: functionsTable
#| warning: false

funspotr::list_files_wd("analysis") |> 
  spot_funs_files() |> 
  unnest_results() |> 
...
An index of all images:

```{r}
#| label: images
#| warning: false

image_files <- list.files(path = "docs/figure",
                          recursive = TRUE,
                          pattern = "*.png$",
                          full.names = TRUE) |> 
  fs::path() |> 
...
matrix(image_files$value, ncol = 4, byrow = TRUE) |>
  as_tibble() |> 
  gt::gt() |> 
