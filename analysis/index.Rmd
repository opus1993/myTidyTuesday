...
articles <- tibble(file = dir(path = "analysis")) %>% 
  filter(!file %in% c("_site.yml","about.Rmd","index.Rmd","license.Rmd"))
...
articles <- articles %>% 
  ...
  mutate(title = stringr::str_sub(title, start = 9L,
                                  stringr::str_length(title) - 1L)) %>% mutate(dateline = stringr::str_sub(dateline, start = 8L,
                                  stringr::str_length(dateline) - 1L)) %>% 
  mutate(libraries = stringr::str_sub(libraries, start = 9L,
                                  stringr::str_length(libraries) - 1L)) %>%  filter(!libraries %in% c("tidyverse","tidytuesdayR","lubridate") ) %>% 
